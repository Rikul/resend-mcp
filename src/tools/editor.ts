...
export function addEditorTools(
  ...
  { registerTools = true }: { registerTools?: boolean } = {},
) {
  ...
  // The editor session helper above is always available so broadcast/template
  // compose tools keep working; the editor tools themselves are only exposed
  // when the Editor category is enabled.
  if (registerTools) {
    server.registerTool(
